...
# Group subnets by AZ
locals {
  public_subnets_by_az  = { for s in var.public_subnets  : s.availability_zone => [] }  
  private_subnets_by_az = { for s in var.private_subnets : s.availability_zone => [] }

  # Populate the lists
  public_subnets_by_az_filled = { for az, _ in local.public_subnets_by_az  : az => [for s in var.public_subnets : s if s.availability_zone == az] }
  private_subnets_by_az_filled = { for az, _ in local.private_subnets_by_az : az => [for s in var.private_subnets : s if s.availability_zone == az] }
}

# Public Route Tables (per AZ)
resource "aws_route_table" "public" {
  for_each = local.public_subnets_by_az_filled
...
resource "aws_route_table_association" "public" {
  for_each = { for s in flatten(values(local.public_subnets_by_az_filled)) : s.id => s }
...
resource "aws_route_table" "private" {
  for_each = local.private_subnets_by_az_filled
...
resource "aws_route_table_association" "private" {
  for_each = { for s in flatten(values(local.private_subnets_by_az_filled)) : s.id => s }
